...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var tipText:FlxText = new FlxText(4, FlxG.height - 32, 0, 'Press ESC to Go Back to Chart Editor', 16);
		tipText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(3.3, 1)), daNoteData, oldNote, true);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (ClientPrefs.RatingCombo)
		rating.loadGraphic(Paths.image(pixelShitPart1 + daRating + pixelShitPart2));
		rating.screenCenter();
		rating.x = coolText.x - 40;
		rating.y -= 60;
		rating.velocity.y -= 50;
		rating.visible = !ClientPrefs.hideHud;
		rating.x += ClientPrefs.comboOffset[0];
		rating.y -= ClientPrefs.comboOffset[1];

		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'combo' + pixelShitPart2));
		if (ClientPrefs.RatingCombo == true)
		comboSpr.screenCenter();
		comboSpr.x = coolText.x;
		comboSpr.velocity.y -= 50;
	...
	function spawnNoteSplashOnNote(note:Note) {
		if(ClientPrefs.RatingCombo && note != null) {
