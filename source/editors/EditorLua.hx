...
class EditorLua {
	...
	public function new(script:String) {
		...
		if(resultStr != null && result != 0) {
			lime.app.Application.current.window.alert(resultStr, 'CORRUPT IS GOING TO YOUR DESTINATION!!!');
			trace('ERROR ON LUA SCRIPT!!' + resultStr);
			lua = null;
			return;
		}
		trace('Script succesfully loaded: ' + script);
	...
	public function call(event:String, args:Array<Dynamic>):Dynamic {
		...
		if(result != null && resultIsAllowed(lua, result)) {
			...
			if(Lua.type(lua, -1) == Lua.LUA_TSTRING) {
				var error:String = Lua.tostring(lua, -1);
				Lua.pop(lua, 1);
				if(error == 'Attempting to call global function or a nil value type is not permitted') { //Makes it ignore warnings and not break stuff if you didn't put the functions on your lua file
					return Function_Continue;
				}
			}
