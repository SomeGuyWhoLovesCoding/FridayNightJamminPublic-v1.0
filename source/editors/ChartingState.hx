...
class ChartingState extends MusicBeatState
{
	...
	[
		['', "a Simple Event that does nothing, you fucking dumbass."],
		['Hey!', "Plays the \"Hey!\" animation from Bopeebo,\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend,\nSomething else = Both.\nValue 2: Custom animation duration,\nleave it blank for 0.6s"],
		['Set GF Speed', "Sets GF head bopping speed,\nValue 1: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\nUsed on Fresh during the beatbox parts.\n\nWarning: Value must be integer!"],
		['Blammed Lights', "Value 1: 0 = Turn off\n1 = Blue\n1 = Green\n4 = Pink\n5 = Red\n6 = Orange\n7 = Cyan\n8 = Brown\n9 = Yellow\n10 = Navy\n11 = Lime\n12 = Fade out all the Sprites."],
		['Kill Henchmen', "For Mom's songs, don't use this please, i love them :("],
		['Add Camera Zoom', "Used on MILF on that one \"hard\" part\nValue 1: Camera zoom add (Default: 0.015)\nValue 2: UI zoom add (Default: 0.18)\nLeave the values blank if you want to use Default."],
		...
		['Character Glow', "Makes The Character have Moodpowers. (FestivalV is recommended)\nValue 1: Mood"],
	...
	**/
	...
	override function create()
	{
		...
		var eventIcon:FlxSprite = new FlxSprite(-GRID_SIZE - 18, -132).loadGraphic(Paths.image('eventArrow'));
		P1Icon = new HealthIcon('bf');
		P2Icon = new HealthIcon('dad');
		P3Icon = new HealthIcon('gf');
		P4Icon = new HealthIcon('soon');

		eventIcon.setGraphicSize(30, 30);
		P1Icon.setGraphicSize(0, 45);
		P2Icon.setGraphicSize(0, 45);
		P3Icon.setGraphicSize(0, 45);
		P4Icon.setGraphicSize(0, 75);
		...
		P1Icon.setPosition(GRID_SIZE * 9.5, 72);
		P2Icon.setPosition(GRID_SIZE * 13.5, 72);
		P3Icon.setPosition(GRID_SIZE * 17.5, 72);
		P4Icon.setPosition(GRID_SIZE * 22, 69);
		...
		FlxG.save.bind('Jammin', 'SaveData');
		...
		bpmTxt = new FlxText(10, 400, 0, "", 16);
	...
	function addSongUI():Void
	{
		...
		var clear_events:FlxButton = new FlxButton(225, 310, 'Clear events', function()
			{
		...
		var clear_notes:FlxButton = new FlxButton(225, clear_events.y + 30, 'Clear notes', function()
			{
		...
		var stepperBPM:FlxUINumericStepper = new FlxUINumericStepper(10, 70, 1, 1, 0.00001, 50000.00000, 1);
		...
		tab_group_song.add(new FlxText(player3DropDown.x, player3DropDown.y - 15, 0, 'Bopper:'));
		...
		tab_group_song.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 0, 'Note Splash Skin:'));
	...
	function addChartingUI() {
		...
		check_vortex = new FlxUICheckBox(10, 160, null, null, "Chart quants", 100);
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 17, Std.int(GRID_SIZE * _song.notes[curSection].lengthInSteps * zoomList[curZoom]));
		...
		var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height / 2).makeGraphic(Std.int(GRID_SIZE * 2), Std.int(gridBG.height), FlxColor.BLACK);
		gridBlack.alpha = 0;
		...
		var gridP3BlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 8)).makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridP3BlackLine);

		var gridP4BlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 12)).makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridP4BlackLine);
		...
		gridP3BlackLine = new FlxSprite(gridBG.x + GRID_SIZE).makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridP3BlackLine);
		updateGrid();

		gridP4BlackLine = new FlxSprite(gridBG.x + GRID_SIZE).makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridP4BlackLine);
		updateGrid();
	}
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			waveformSprite.makeGraphic(Std.int(GRID_SIZE * 12), Std.int(gridBG.height), 0x00FFFFFF);
	...
	function updateHeads():Void
	{
		...
		{
			P1Icon.changeIcon(healthIconP1);
			P2Icon.changeIcon(healthIconP2);
			P3Icon.changeIcon(healthIconP3);
		}
		else
		{
			P1Icon.changeIcon(healthIconP2);
			P2Icon.changeIcon(healthIconP1);
			P3Icon.changeIcon(healthIconP3);
		}
	}
