...
// Thanks to Theodoros Theodorou for shader support (This file is fucking before compilation lmfao)
...
class ChromaticAberrationEffect extends Effect
{
  ...
  public function new(offset:Float = 0.00){
    ...
    shader.rOffset.value = [0.002];
    shader.gOffset.value = [0];
    shader.bOffset.value = [-0.002];
  }
...
class GrainEffect extends Effect {
	...
	public function new (grainsize, lumamount,lockAlpha){
		...
		shader.lockAlpha.value = [lockAlpha];
		shader.uTime.value = [FlxG.random.float(0,3)];
    ...
    vec4 getVideo(vec2 uv)
      {
        ...
        if(distortionOn){
        	...
        	look.x = look.x + (sin(look.y*5. + iTime)/50.*onOff(4.,4.,.3)*(1.+cos(iTime*80.))*window)*(glitchModifier*2);
...
class PulseEffect extends Effect
{
    ...
    public var waveSpeed(default, set):Float = 2.45138;
	public var waveFrequency(default, set):Float = 0.05;
	public var waveAmplitude(default, set):Float = 0.01;
	...
	public function new(waveSpeed:Float,waveFrequency:Float,waveAmplitude:Float):Void
	{
		...
		this.waveFrequency = waveFrequency;
		this.waveAmplitude = waveAmplitude;
		shader.uTime.value = [0.02];
        shader.uampmul.value = [0.02];
        shader.uEnabled.value = [true];
		PlayState.instance.shaderUpdates.push(update);
	}
