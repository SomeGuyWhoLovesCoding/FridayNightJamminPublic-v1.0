...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "YOU HAVE NO MICROMODS INSTALLED!!\nPRESS ENTER TO GO TO MY\nUSERPAGE TO FIND A MICROMOD!!", 48);
		if(FlxG.random.bool(0.1)) noModsTxt.text += '\nBITCH.'; //meanie
		noModsTxt.setFormat(Paths.font("vcr.ttf"), 35, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		removeButton = new FlxButton(startX, 620, "Delete Selected Mod", function()
		{
			...
			{
				trace('Fucking Deleting Path: ' + path);
				try
				{
				...
				{
					trace('We've Detected a Critical Error: ' + e);
				}
			}
		});
	...
	function onLoadError(_):Void
	{
		...
		_file = null;
		canExit = true;
		trace("Trust me, we are missing the file.");
	}*/
}
